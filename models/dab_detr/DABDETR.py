...
class DABDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # default pipeline
        embedweight = self.refpoint_embed.weight  # [num_queries, query_dim]
        hs, reference = self.transformer(self.input_proj(src), mask, embedweight, pos[-1])
        
        
        # 根据是否每层使用不同边界框嵌入采取不同处理逻辑
        # bbox_embed_diff_each_layer=False: 所有解码器层共享同一个边界框嵌入网络
        # bbox_embed_diff_each_layer=True: 每个解码器层使用独立的边界框嵌入网络
        if not self.bbox_embed_diff_each_layer:
            # 将参考点坐标从sigmoid空间转换回原始空间（反sigmoid变换）
            reference_before_sigmoid = inverse_sigmoid(reference)
            # 通过边界框嵌入网络处理Transformer输出
            tmp = self.bbox_embed(hs)
            # 将参考点坐标添加到边界框预测中（残差连接）
            tmp[..., :self.query_dim] += reference_before_sigmoid
            # 对输出坐标应用sigmoid激活，归一化到[0,1]范围
